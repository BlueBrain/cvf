...
class Config:
    ...
    def _fill_useion_template(self):
        ...
        try:
            ...
            if "WRITE" in self.mod.data["NEURON"]["USEION"]:
                self.data["channel"]["USEION"]["WRITE"] = self.mod.data["NEURON"]["USEION"]["WRITE"]
            if "READ" in self.mod.data["NEURON"]["USEION"]:
                self.data["channel"]["USEION"]["READ"] = dict(zip(self.mod.data["NEURON"]["USEION"]["READ"], [self.data["channel"]["USEION"]["READ"]]*len(self.mod.data["NEURON"]["USEION"]["READ"])))
    ...
    def _purify_raw_in(self):
        ...
        try:
            ...
            del [self.data["channel"]["revName"], self.data["channel"]["revValue"], self.data["channel"]["current"]]
        except KeyError:
            pass
